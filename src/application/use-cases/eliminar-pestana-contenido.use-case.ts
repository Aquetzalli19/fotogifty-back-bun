...
export class EliminarPestanaContenidoUseCase {
  constructor(
  ...
  async execute(configId: number, sectionId: number, pestanaId: number): Promise<Result> {
    try {
      ...
      await this.pestanaRepo.delete(pestanaId);
      return { success: true, message: 'Pestaña eliminada exitosamente' };
    } catch (error: any) {
      return { success: false, message: error.message || 'Error al eliminar pestaña' };
    }
  }
}
